...
//Handler de eventos para eventos de WiFi
                                ...
                                int32_t event_id, void* event_data)
{
    ...
    //se ingresa al if para intentar reconectarse
        ...
        //En caso de que se supere el número máximo de intentos, se le informa al loop del event group que hubo un fallo en la
        //conexión
        else {
...
//Handler de eventos para eventos de IP
...
esp_err_t connect_wifi(wifi_network_t* wifi_network)
{
    ...
    //Se crea un "evento loop" estandar, que es un ciclo interno que se utiliza para controlar eventos, como por ejemplo, conexiones
    //o desconexiones a red WiFi, o llegada de datos
    ESP_ERROR_CHECK(esp_event_loop_create_default());

    //Se le indica a la interfaz de red que vamos a utilizar WiFi y en el modo STA, con la configuración estándar o default
    esp_netif_create_default_wifi_sta();

    //Se crea una variable mediante la cual se realiza la configuración para el inicio de los parametros del WiFi, en este caso, 
    //se obtiene la configuración estándar
    ...
    //Se crea una variable que representa una instancia del handler de eventos de WiFi, que luego puede utilizarse para cerrar 
    //dicho handler
    ...
    //Luego, sea que hubo un error o se estableció la conexión de forma exitosa, nos desuscribimos del "event group" y de los
    //handlers
